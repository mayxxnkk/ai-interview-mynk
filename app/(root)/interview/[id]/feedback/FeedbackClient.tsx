...
const FeedbackClient = ({ interviewId, serverFeedback }: Props) => {
    const [feedback, setFeedback] = useState<Feedback | null>(serverFeedback);

    useEffect(() => {
        if (!feedback) {
            // Try sessionStorage fallback
            const stored = sessionStorage.getItem(`feedback_${interviewId}`);
            if (stored) {
                try {
    ...
    if (!feedback) {
        return (
            <div className="section-feedback">
                ...
                <Link href={`/interview/${interviewId}`} className="btn-primary px-6 py-3 rounded-full text-center w-fit">
                    Take Interview
                </Link>
            </div>
        );
                    ...
                    {feedback.categoryScores?.map((cat, i) => (
                        <div key={i} className="flex flex-col gap-2 p-5 dark-gradient rounded-2xl border border-light-600/20">
                            ...
                            <div className="w-full bg-dark-300 rounded-full h-2">
                                <div className="progress rounded-full h-2" style={{ width: `${cat.score}%` }} />
                            </div>
